 ...
 * {@link #setNumReduceTasks(int)}), some parameters interact subtly with the
 * rest of the framework and/or job-configuration and is relatively more complex
 * for the user to control finely (e.g. {@link #setNumMapTasks(int)}).</li>
 * </ol>
 * 
 * <p>
...
public class JobConf extends Configuration {
	...
	private static final Pattern JAVA_OPTS_XMX_PATTERN = Pattern
			.compile(".*(?:^|\\s)-Xmx(\\d+)([gGmMkK]?)(?:$|\\s).*");
	 ...
	 * that the options are turned off. Deprecated because it makes no sense in
	 * the context of MR2.
	 */
	@Deprecated
	...
	/**
	 * @deprecated Configuration key to set the maximum virtual memory available
	 *             to the child map and reduce tasks (in kilo-bytes). This has
	 *             been deprecated and will no longer have any effect.
	 */
	@Deprecated
	...
	/**
	 * @deprecated Configuration key to set the maximum virtual memory available
	 *             to the map tasks (in kilo-bytes). This has been deprecated
	...
	/**
	 * @deprecated Configuration key to set the maximum virtual memory available
	 *             to the reduce tasks (in kilo-bytes). This has been deprecated
	 *             and will no longer have any effect.
	 */
	@Deprecated
	 ...
	 * reference existing environment variables via <code>$key</code> on Linux
	 * or <code>%key%</code> on Windows.
	 * 
	 * Example:
	 * <ul>
	...
	/**
	 * The variable is kept for M/R 1.x applications, M/R 2.x applications
	 * should use {@link MRJobConfig#WORKFLOW_ID} instead
	 */
	@Deprecated
	...
	/**
	 * The variable is kept for M/R 1.x applications, M/R 2.x applications
	 * should use {@link MRJobConfig#WORKFLOW_NAME} instead
	 */
	@Deprecated
	...
	/**
	 * The variable is kept for M/R 1.x applications, M/R 2.x applications
	 * should use {@link MRJobConfig#WORKFLOW_NODE_NAME} instead
	 */
	@Deprecated
	public static final String WORKFLOW_NODE_NAME = MRJobConfig.WORKFLOW_NODE_NAME;

	/**
	 * The variable is kept for M/R 1.x applications, M/R 2.x applications
	 * should use {@link MRJobConfig#WORKFLOW_ADJACENCY_PREFIX_STRING} instead
	 */
	@Deprecated
	public static final String WORKFLOW_ADJACENCY_PREFIX_STRING = MRJobConfig.WORKFLOW_ADJACENCY_PREFIX_STRING;

	/**
	 * The variable is kept for M/R 1.x applications, M/R 2.x applications
	 * should use {@link MRJobConfig#WORKFLOW_ADJACENCY_PREFIX_PATTERN} instead
	 */
	@Deprecated
	public static final String WORKFLOW_ADJACENCY_PREFIX_PATTERN = MRJobConfig.WORKFLOW_ADJACENCY_PREFIX_PATTERN;

	/**
	 * The variable is kept for M/R 1.x applications, M/R 2.x applications
	 * should use {@link MRJobConfig#WORKFLOW_TAGS} instead
	 */
	@Deprecated
	...
	public InputFormat getInputFormat() {
		Class<? extends InputFormat> class1 = getClass(
				"mapred.input.format.class", TextInputFormat.class,
				InputFormat.class);

		return ReflectionUtils.newInstance(class1, this);
	}
	...
	public OutputFormat getOutputFormat() {
		Class<? extends OutputFormat> class1 = getClass(
				"mapred.output.format.class", TextOutputFormat.class,
				OutputFormat.class);
		return ReflectionUtils.newInstance(class1, this);
	}
	 ...
	 * keys of inputs to the combiner.
	 *
	...
	public RawComparator getCombinerKeyGroupingComparator() {
		Class<? extends RawComparator> theClass = getClass(
				JobContext.COMBINER_GROUP_COMPARATOR_CLASS, null,
				RawComparator.class);
		if (theClass == null) {
	 ...
	 *
	 * <p>
	 * <i>Note</i>: This is not a guarantee of the combiner sort being
	...
	public void setCombinerKeyGroupingComparator(
			Class<? extends RawComparator> theClass) {
		setClass(JobContext.COMBINER_GROUP_COMPARATOR_CLASS, theClass,
				RawComparator.class);
	}
   ...
   * <a href="{@docRoot} multiplied by (&lt;<i>no. of nodes</i>&gt; * <a
	 ...
	 * 
	 * @param hostadd
	 *            the JobSubmitHostAddress for this job.
	 */
	 ...
	 * returns {@link JobContext#DEFAULT_MAP_MEMORY_MB}.
	 * <p>
	 ...
	 * 
	 * If a value is specified in the configuration, it is returned. Else, it
	 * returns {@link JobContext#DEFAULT_REDUCE_MEMORY_MB}.
	 * <p>
	 * For backward compatibility, if the job configuration sets the key
	 * {@link #MAPRED_TASK_MAXVMEM_PROPERTY} to a value different from
	 * {@link #DISABLED_MEMORY_LIMIT}, that value will be used after converting
	 ...
	 * <p>
	 * For backward compatibility, if the job configuration sets the key
	 * {@link #MAPRED_TASK_MAXVMEM_PROPERTY}, that value is returned. Otherwise,
	 * this method will return the larger of the values returned by
	...
	public void setMaxVirtualMemoryForTask(long vmem) {
		...
		if (vmem < 0) {
			throw new IllegalArgumentException(
					"Task memory allocation may not be < 0");
		}
	...
	/**
	 * Parse the Maximum heap size from the java opts as specified by the -Xmx
	 * option Format: -Xmx&lt;size&gt;[g|G|m|M|k|K]
	 * 
	 * @param javaOpts
	 *            String to parse to read maximum heap size
	...
	private int getMemoryRequiredHelper(String configName, int defaultValue,
			int heapSize, float heapRatio) {
